    ...
    <div>
      <div className="grid sm:grid-cols-2 grid-cols-1 gap-3 text-gray-50">
        {currentGames.map((minigame: any, index: number) => (
          <div
            key={minigame.id}
            style={{ backgroundImage: `url(${minigame.image})` }}
            className={`bg-cover bg-center rounded-lg flex bg-white dark:bg-gray-800 shadow-[0_0_10px_rgba(0,0,0,0.25)] overflow-hidden ${index === 0 ? "sm:row-span-2" : "h-[150px] "}`}
          >
            <div className="p-4 size-full bg-gradient-to-t from-gray-900/80 via-gray-900/60 to-transparent flex flex-col justify-end">
              ...
              <Tippy content={minigame.description}>
                <p className={`${index === 0 ? "line-clamp-2" : "line-clamp-1"} mt-1 text-sm cursor-pointer`}>{minigame.description}</p>
              </Tippy>
