...
const DiscordOnline: React.FC = () => {
  const [serverData, setServerData] = useState<ServerData | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<any>("https://discord.com/api/guilds/1063302010918678538/embed.json");
  ...
  return (
    <div className='p-3 bg-[#5865F2] rounded-lg shadow-[0_0_10px_rgba(0,0,0,0.25)]'>
      <svg width='150' viewBox='0 0 292 80' fill='none' xmlns='http://www.w3.org/2000/svg'>
        <g clipPath='url(#clip0)'>
          <g clipPath='url(#clip1)'>
            ...
            <path
              ...
              fill='white'
            ></path>
            <path d='M134.472 36.5435C137.478 37.8679 141.208 37.9265 144.283 36.5435V55.0154H134.472V36.5435Z' fill='white'></path>
          </g>
        </g>
      </svg>
      <div className='mt-3 text-gray-50 flex justify-between items-center'>
        ...
        <a className='size-[30px] rounded-full bg-gray-50 text-[#5865F2] flex items-center justify-center' aria-label='Discord' href='https://discord.gg/cubecraft' target='_blank' rel='noopener'>
...
const MoreDiscordOnline: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<ServerData>("https://discord.com/api/guilds/1063302010918678538/embed.json");
  ...
  return (
    <div className='mt-5 bg-white dark:bg-gray-800 rounded-lg shadow-[0_0_10px_rgba(0,0,0,0.25)]'>
      ...
      <div className='px-[15px]'>
        {serverData ? (
          serverData.members.slice(0, 10).map((member) => (
            <div className='flex items-center py-3 gap-4' key={member.id}>
              <Image className='size-8 rounded-sm' src={member.avatar_url} alt={`${member.username}`} />
              <div className='text-sm text-gray-800 dark:text-gray-300'>
                <p className='text-green-700 dark:text-green-500'>{member.username}</p>
                <p className='text-xs'>{statusTextMap[member.status]}</p>
              </div>
            </div>
          ))
        ) : (
        ...
        <Link className='w-full block' to='/members'>
          <span className='w-full block mb-2 text-right text-xs text-gray-600 dark:text-gray-300 underline'>Xem thêm</span>
        </Link>
      </div>
      <div className='px-[15px] py-3 bg-gray-100 dark:bg-gray-700/50 rounded-bl-lg rounded-br-lg border-t dark:border-t-gray-600 border-t-gray-400'>
        <p className='text-gray-600 dark:text-gray-300 text-xs'>{serverData ? <>Trực tuyến: {serverData.presence_count}</> : <>Trực tuyến: 0</>}</p>
      </div>
    </div>
  );
};

export { MoreDiscordOnline, DiscordOnline };
